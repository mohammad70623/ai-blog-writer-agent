...
def try_stream(inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """Stream graph updates, fall back to invoke if streaming fails."""
    try:
...
# ──────────────────────────────────────────────
# Markdown renderer that handles local images
# ──────────────────────────────────────────────
...
def render_markdown_with_local_images(md: str):
    ...
    while i < len(parts):
        ...
        # Check if next part is a caption line
        if i + 1 < len(parts) and parts[i + 1][0] == "md":
            nxt = parts[i + 1][1].lstrip()
            first_line = nxt.splitlines()[0].strip() if nxt.strip() else ""
            mc = _CAPTION_RE.match(first_line)
            if mc:
...
with st.sidebar:
    ...
    topic = st.text_area("Topic", height=120, placeholder="e.g. How RAG pipelines work in production")
    ...
    if not past_files:
        ...
        if st.button("📂 Load selected blog"):
            if selected_md_file:
                loaded_md = read_md_file(selected_md_file)
...
if run_btn:
    ...
    st.session_state["logs"] = []  # reset logs for new run
    ...
    for kind, payload in try_stream(inputs):
        if kind == "updates":
            ...
            st.session_state["logs"].append(f"[{kind}] {json.dumps(payload, default=str)[:1200]}")

        elif kind == "final":
            st.session_state["last_out"] = payload
            status.update(label="✅ Done!", state="complete", expanded=False)
            st.session_state["logs"].append("[final] Graph completed.")
...
if out:
    ...
    with tab_plan:
        ...
        if not plan_obj:
            st.info("No plan found (loaded from file or closed-book run).")
        else:
    ...
    with tab_evidence:
        ...
        if not evidence:
            st.info("No evidence (closed_book mode or no Tavily API key).")
        else:
            rows = []
            for e in evidence:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            st.download_button(
                ...
                data=bundle_zip(final_md, md_filename, Path("data/images")),
    ...
    with tab_images:
        ...
        if not specs and not images_dir.exists():
            ...
            if specs:
                with st.expander("Image plan (JSON)"):
                    st.json(specs)
